...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{../assets/metrics_plot.png}
\caption{Macro-averaged evaluation metrics on the training set (4 patients, 73 ground-truth conditions).}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Macro-averaged evaluation results on the training set.}
\begin{tabular}{lcc}
\toprule
...
\begin{itemize}[leftmargin=1.2em, itemsep=1pt]
  \item \textbf{Evidence recall (0.96)} is the strongest metric --- the deterministic hardening pass effectively fills evidence gaps across notes that the LLM missed.
  \item \textbf{Status accuracy (0.94)} validates the chronological resolution strategy (latest note wins).
  \item \textbf{Onset exact (0.72)} is the weakest metric, consistent with the known difficulty of resolving relative date expressions (``since mid-December'') to absolute dates.
  \item \textbf{Condition recall (0.91) $>$ precision (0.86)} reflects the design bias toward over-extraction followed by consolidation.
\end{itemize}
